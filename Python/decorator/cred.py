...
items=[{"id":100,"name":"cb","price":160},
...
# def retrieve_item(id=None,*args,**kwargs):
    # obj=[i for i in items if i.get("id")==id] #to pick one
    # print(obj)
# retrieve_item(id=105)


# def destroy_item(id=None,*args,**kwargs):
#     obj=[i for i in items if i.get("id")==id][0]   delete
#     items.remove(obj)
...
def update_item(id=None,*args,**kwargs):
    obj=[i for i in items if i.get("id")==id][0]   #update
    obj.update(kwargs)

# update_item=(id=103,name="GHEE ROAST")
print(retrive_item(id=103))
